
# 🧡 KrabbelNet DNA

> Het internet mag weer gezellig zijn.

Dit document bevat de ontwerpprincipes en UX-ideeën die KrabbelNet onderscheiden. Niet alles wordt direct gebouwd, maar alles in dit document moet bijdragen aan het gevoel van KrabbelNet.

---

# Ontwerpprincipes

- Mensen boven functies.
- Gezelligheid boven engagement.
- Rust boven drukte.
- Privacy is vanzelfsprekend.
- Begroet mensen als vrienden, niet als gebruikers.
- Vier kleine momenten.
- Laat gebruikers glimlachen.

---

# Welkomstgroeten

## Tijdgebonden begroetingen

🌅 06:00 – 12:00
- Goedemorgen, {naam}!

☀️ 12:00 – 18:00
- Goedemiddag, {naam}!

🌙 18:00 – 23:00
- Goedenavond, {naam}!

🌃 23:00 – 06:00
- Hé nachtbraker, {naam}!

---

# Wisselende welkomstregels

- 🧡 Leuk dat je er weer bent.
- 💬 Tijd voor een nieuwe krabbel?
- 😊 Benieuwd wat je vrienden vandaag hebben gedeeld?
- ☕ Pak een kop koffie en kijk eens rond.
- 🎯 Wat speelt er vandaag?
- ✨ Gezellig dat je langskomt.

---

# Te vieren momenten

- 🎉 Account aangemaakt
- 💬 Eerste krabbel
- 👥 Eerste vriend
- 🎂 Verjaardag
- 🏆 100e login

---

# Ideeën voor later

- 🎄 Kerstbegroeting
- 🎆 Oud & Nieuw
- 🐣 Pasen
- 🌞 Zomer-thema
- 🍂 Herfst-thema
- ❄️ Winter-thema

---

# UX-regel

Elke nieuwe functie moet bijdragen aan het gevoel:

"Het internet mag weer gezellig zijn."
💛 Animatieprincipes
Animaties duren maximaal 1 seconde.
Animaties mogen warmte toevoegen, nooit afleiden.
Het logo mag één rustige hartslag geven bij binnenkomst.
Nieuwe content mag zacht verschijnen (fade/slide).
Geen schreeuwerige of verslavende effecten.
Ik merk trouwens iets leuks: we beginnen een eigen ontwerptaal te ontwikkelen.

🧡 Het logo klopt één keer bij binnenkomst.
❤️ Het hartje bij "Vriend toevoegen" klopt rustig.
👋 Begroetingen zijn persoonlijk.
💬 De interface blijft rustig.
# KrabbelNet DNA

- Het internet mag weer gezellig zijn.
- Mensen boven algoritmes.
- Profielen zijn persoonlijk.
- Vriendschap is wederzijds.
- Kleine warme details maken het verschil.
- Privacy is vanzelfsprekend.
